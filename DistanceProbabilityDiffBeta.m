...
B_ScalarValues = [0,1,10,100,1000];

foodSourceXPos = 20; %X Pos Of Food Source
foodSourceYPos = 20; %Y Pos Of Food Source

nestPositions = 12:1:22;
excludefoodSourcePositions = [foodSourceXPos, 40];
chosenfoodSourcePositions = ~ismember(nestPositions, excludefoodSourcePositions);
finalNestPositions = nestPositions(chosenfoodSourcePositions);
...
ProbCColony1 = zeros(length(finalNestPositions),1);
DistanceRatio = zeros(1, length(finalNestPositions));


for bScalar = 1: length(B_ScalarValues)

    B_Scalar1 = B_ScalarValues(bScalar); %Strength Of Pheromone;
    B_Scalar2 = 55; %Strength Of Pheromone;

    for jk = 1: length(finalNestPositions)
        ...
        for nn = 1: NumSimulations %Total Number Of Simulations
            ...
            while (t < T)
                ...
                % Colony 1: Lines 228-481
                % Colony 2: Lines 485-741
                % *** Choose A Random Ant
                ...
                for j = 1:N %N Ant Movements
                    ...
                    for jj = 1:8 %Assign Weights For Each Position Ant Can Move In Based On Difference In Pheromone Concentration
                        if du(jj)< 0
                ...
                for l = 1:S %S Ant Movements
                    ...
                    for ll = 1:8  %Assign Weights For Each Position Ant Can Move In Based On Difference In Pheromone Concentration
                        if dr(ll)< 0
                ...
                % Same Process For Colonies 1 And 2
                % Colony 1: Lines 756-767
                % Colony 2: Lines 769-780
                % *** Obtain Ant's Position
                % *** Check If Ant Is Forager OR Returner
                % *** If Ant Is Forager, Add Ant To Matrix A
                % *** If Ant Is Returner, Add Ant To Matrix B


                % for j = 1:N(jk)
                %     r = ant_position(j,1);
                %     c = ant_position(j,2);
                %     if (ant_position(j,3) == 0) %Check If Ant Is Forager
                %         A(r,c) = A(r,c) + 1;    %Adding Ant To Same Position On Different Matrix (Matrix A, Forager); NOTE: More Than One Ant Can Occupy This Position (Hence, +1)
                %     else %Ant Is Returner
                %         r = floor(r);
                %         c = floor(c);
                %         B(r,c) = B(r,c) + 1; %Move Ant to Same Position On Different Matrix (Matrix B, Returner); NOTE: More Than One Ant Can Occupy This Position (Hence, +1)
                %         addNumReturners1 = addNumReturners1 + 1;
                %
                %     end
                %
                %     if (addNumReturners1 == 1)
                %         TimeFoodFoundColony1(nn) = t;
                %     end
                % end
                ...
                %     e = ant1_position(l,1);
                %     f = ant1_position(l,2);
                %     if (ant1_position(l,3) == 0) %Check If Ant Is Forager
                %         A(e,f) = A(e,f) + 1; %Adding Ant To Same Position On Different Matrix (Matrix A, Forager); NOTE: More Than One Ant Can Occupy This Position (Hence, +1)
                %     else %Ant Is Returner
                %         e = floor(e);
                %         f = floor(f);
                %         B(e,f) = B(e,f) + 1; %Move Ant to Same Position On Different Matrix (Matrix B, Returner); NOTE: More Than One Ant Can Occupy This Position (Hence, +1)
                %         addNumReturners2 = addNumReturners2 + 1;
                %     end
                %
                %     if (addNumReturners2 == 1)
                %         TimeFoodFoundColony2(nn) = t;
                %     end
                % end
            ...
            elseif(Q >= .25*S && J >= 0.25*N) %Checking: Both Colonies 1 And 2 Have More Than 25 Returners, Then Another Trail Has Been Formed By Both Colonies
                Colony1FormedTrail(nn) = 1;
                Colony2FormedTrail(nn) = 1;
            elseif(J < 0.25*N && Q < 0.25*S) %Checking: Both Colonies Have Less Than 50 Returners, Neither Colony Has Formed A Trail
                Colony1FormedTrail(nn) = 0;
                Colony2FormedTrail(nn) = 0;
            end
        ...
        ProbBothColoniesTrail = (sum(Colony1FormedTrail & Colony2FormedTrail))/(NumSimulations); %Probability Of Both Colonies

        ProbOnlyColony1Trail = (sum((Colony1FormedTrail == 1) & (Colony2FormedTrail == 0)))/ (NumSimulations); %Probability Of Only Colony 1

        ProbOnlyColony2Trail = (sum((Colony2FormedTrail == 1) & (Colony1FormedTrail == 0)))/ (NumSimulations); %Probability Of Only Colony 2
        ...
        ProbJustColony1(jk) = ProbOnlyColony1Trail;
        ProbCColony1(jk) = ProbColony1Trail;

        % \\labels = {'Only Colony 1', 'Only Colony 2', 'Colony 1', 'Colony 2', 'Not Colony 1', 'Not Colony 2', 'Both', 'Neither'};
        % values = [ProbOnlyColony1Trail, ProbOnlyColony2Trail, ProbColony1Trail, ProbColony2Trail, ProbNoColony1Trail, ProbNoColony2Trail, ProbBothColoniesTrail, ProbNeitherColoniesTrail];
        %
        %
        % figure(4)
        % bb = bar(values, 'FaceColor', 'flat');
        %
        % % Use a rainbow colormap (you can try hsv, turbo, parula, etc.)
        % rainbowColors = jet(length(values)); % or hsv(length(values)) for hue cycle
        % bb.CData = rainbowColors;
        %
        % % Format axes
        % ax = gca;
        ...
        % y1 = ylabel('Probability Of Trail Formation')
        % y1.Position(1) = y1.Position(1) - 0.16;  % Move left
        % y1.Position(2) = y1.Position(2) + 0.1
        % ylim([0 1.2])
        % Title = title('Equal Probability')
        % Title.Units = "normalized"
        % Title.Position(2) = 1.02
        % grid on
        %
...
% figure(4)
% x = DistanceRatio;
% %y = AverageTimeFoodFound;
% y = ProbJustColony1;
%
% % sem = std_dev / sqrt(nm); % Calculate standard error of the mean
%
% % sem = zeros(1, length(y));
% %
% % for st = 1: length(y)
% %     sem(st) = sqrt((y(st)*(1 - y(st)))/length(y)); % How the means differs from each probability value
% % end
% %
% %
% % err = zeros(size(y));
...
% xlabel('Distance Ratio (Colony 1 to Colony 2) ');
% ylabel('Probability Of Trail Formation');
% %title('How does the distance affect the time it takes to find a food source?');
% drawnow
%
% % figure_handle = gcf; % Get handle to the current figure
% % saveas(figure_handle, 'myfigure.eps', 'epsc'); % Saves in color EPS
