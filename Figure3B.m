...
S = 1000; % Number Of Ants In Colony 2
N = 1000:100:10000; %Number Of Ants In Colony 1
M = 50;
...
stdErrorBarValuesProbability = zeros(1, length(N));


ProbJustColony1 = zeros(1,length(N)); %Probability of ONLY Colony 1 Forming A Trail
ProbCColony1 = zeros(1,length(N)); %Probability of Colony 1 Forming A Trail
AntRatio = zeros(1,length(N)); %Ratio Between Colony 1's Distance And Colony 2's Distance


for bScalar = 1: length(B_ScalarValues) %Beta Value Changes For Colony 1

    B_Scalar1 = B_ScalarValues(bScalar); %Strength Of Pheromone For Colony 1;
    B_Scalar2 = 55; %Strength Of Pheromone For Colony 2;

    for jk = 1: length(N) %Colony 1's Nest Changes

        nestXPos = 17; %Nest X Pos Of Ant Colony 1
        nestYPos = nestXPos; %Nest Y Pos Of Ant Colony 1

        nestX2Pos = 22; %Nest X Pos Of Ant Colony 2
        nestY2Pos = nestX2Pos; %Nest Y Pos Of Ant Colony 2
        ...
        AntRatio(jk) = N(jk)/S; %Ratio Between Distance 1 And Distance 2


        for nn = 1: NumSimulations %Total Number Of Simulations

            A = zeros(M,M); %Matrix -> Foragers
            A(nestXPos,nestYPos) = N(jk); %Position Of Ants In Colony 1
            ...
            z = [nestX2Pos - foodSourceXPos, nestY2Pos - foodSourceYPos]; %Distance Between Nest And Food Source For Ant Colony 2
            ...
            while (t < T) %Ants Forage For Food Until Final Time Is Reached
                ...
                for j = 1:N(jk) %N Ant Movements
                    R = randi([1,N(jk)]); % Choosing A Random Ant
            ...
            if (J >= .25 * N(jk) && Q < .25*S) %Checking: Colony 1 Has More Than 25 Returners, Which Is Greater Than The Number Of Returners From Colony 2
                Colony1FormedTrail(nn) = 1;
                Colony2FormedTrail(nn) = 0;
            elseif(Q >= .25*S && J < 0.25*N(jk))%Checking: Colony 2 Has More Than 25 Returners, Which Is Greater Than The Number Of Returners From Colony 1
                Colony2FormedTrail(nn) = 1;
                Colony1FormedTrail(nn) = 0;
            elseif(Q >= .25*S && J >= 0.25*N(jk)) %Checking: Both Colonies 1 And 2 Have More Than 25 Returners, Then Another Trail Has Been Formed By Both Colonies
                Colony1FormedTrail(nn) = 1;
                Colony2FormedTrail(nn) = 1;
            elseif(J < 0.25*N(jk) && Q < 0.25*S) %Checking: Both Colonies Have Less Than 50 Returners, Then Neither Colony Has Formed A Trail
                Colony1FormedTrail(nn) = 0;
                Colony2FormedTrail(nn) = 0;
            end
    ...
    %Plot
    x = AntRatio;
    y(bScalar,:) = ProbJustColony1;
    save("ColonySizeBeta.mat", "x", "y");
